...
Передзаванаж текстури НА ПОЧАТКУ (await): const tex = {}; const KEYS = ["fruit1.webp","fruit2.webp","fruit3.webp","fruit4.webp","fruit5.webp","fruit6.webp","half1.webp","half2.webp","half3.webp","half4.webp","half5.webp","half6.webp","bomb.webp","bg.webp","splat1.webp","splat2.webp","splat3.webp","splat4.webp"]; for (const k of KEYS) tex[k] = await Assets.load({ src: cfg.assets[k], format: "webp" });
- ФОН: Sprite з tex["bg.webp"], розтягни на весь екран (cover) у найнижчому шарі.
- ФРУКТ: випадковий індекс N=1..6; new Sprite(tex["fruitN.webp"]) (це ШКІРКА/зовні), anchor 0.5, масштаб під ~r. ЗАПАМ'ЯТАЙ N на об'єкті — для половинок.
- БОМБА: new Sprite(tex["bomb.webp"]).
- РОЗРІЗ → ПОКАЗАТИ М'ЯКОТЬ (важливо, ефект «3д нутрощів»): цілий фрукт-спрайт (шкірку) ховай. Бери tex["halfN.webp"] (зріз-м'якоть ТОГО Ж фрукта N — насіння/дольки видно). Зроби з нього ДВІ половинки: контейнер обернений на кут свайпу (angle=atan2(dy,dx)), дві Sprite(halfN) з прямокутними Graphics-масками на ліву/праву частину від лінії розрізу. Половинки розлітаються ПЕРПЕНДИКУЛЯРНО до свайпу + гравітація + обертання. Так на зрізі видно соковиту середину, не шкірку.
