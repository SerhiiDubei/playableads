// Standalone labs builder for "Dream Floor" (Stage 1 greybox prototype).
...
//   npx tsx src/assetgen/build-dream-floor.ts
...
const GAME_DIR = path.join(ROOT, "labs", "dream-floor");
const OUT_DIR = path.join(ROOT, "test", "dream-floor");
...
export async function buildDreamFloor(styleId = "dream-floor") {
  ...
  const config: PlayableConfig = {
    copy: { title: "Dream Floor", cta: "Shop Real Flooring" },
    lang: "en",
    style,
...
async function main(): Promise<void> {
  const { outPath, validation, rawBytes } = await buildDreamFloor();
