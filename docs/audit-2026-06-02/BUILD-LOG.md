...
## Bath ONE-ROOM rebuild + power-wash endcard + 5 carpet-варіантів (2026-06-11) 🆕
**Контекст:** критика юзера: (1) bath-morph = 3 РІЗНІ згенеровані кімнати замість одного простору; (2) endcard power-wash «хуйня дичайша» (alpha-панель + дисклеймер поверх фото-стрічки) «при чому ВСЮДИ»; (3) чистка має вимагати 100%; (4) +4-5 копій карпет-версії з різним інвентарем.
**Зроблено:** (1) **gen-bath-carpet-assets.ts**: bath shell-base (порожня кімната) + shell-mid/lux через images.edit (та САМА кімната, інші поверхні) + 12 об'єктів-cutouts (transparent, страйт-он) + 5×(килим full-frame + інструмент cutout) — 25 джобів, усі переглянуто оком. (2) **bath game.ts повний rewrite**: один shell-простір (3 фініші кросфейдом), об'єкти = окремі спрайти у фіксованих слотах (painter zIndex=baseline), 7 порогів ($8K LED дзеркало → $10.5K дуб-тумба → $13K рушникосушка+рослина → $15.5K стіни/підлога → $18K freestanding ванна+табурет → $20.5K floating тумба → $23K мармур+скло-душ), tub слайдиться вбік під душ, OBJ_TWEAKS pixel-фікси (дзеркала над лінією підлоги), endcard стрічка = ЖИВИЙ RenderTexture-знімок кімнати. (3) **power-wash**: snapFill 0.70→0.985 (прибрати все!), endcard переписано на solid cy-cursor патерн, [State]→your state (3 ігри). (4) **carpet-clean-v1..v5**: нова гра (stamp-mask engine, CPU grid 22×30, win 0.985, stubborn spot, foam, shine, auto-demo до 100%, інструмент слідує за пальцем) × 5 варіантів (перський/vacuum, geo/brush, shaggy/steam, kids/sponge, scandi/roller) з пер-варіантними палітрами.
**Перевірено:** 7/7 build validation OK · нові QA-тулзи qa-bath-flow.mjs (8 станів повзунка + lock + endcard) і qa-scrub-flow.mjs (серпантин-скраб до 100%) на ANGLE GPU · ОСОБИСТИЙ перегляд кожного скріншота: bath 8 станів = одна кімната, об'єкти міняються окремо ✓; знайдено й виправлено дзеркало-нижче-лінії-підлоги та порожню endcard-стрічку · power-wash + 5 карпетів: hook/mid/endcard чисті, нуль перетинів.
**Висновок:** конфігуратор-урок (asset-план = кадр/cutout на стан) тепер застосовано і до slider-морфів: shell через images.edit + об'єкти окремо = «один простір». Endcard-патерн (solid, cy-курсор, CTA всередині) — стандарт у power-wash/carpets/bath.
**Відкрито:** endcard-аудит решти 6 ігор (monkey-QA в процесі); deploy gh-pages + push після.

## Dream Kitchen — LAYERED FRAME rebuild (2026-06-11) 🆕
**Контекст:** жорстка критика юзера: фото-матриця повних кадрів = неприйнятно; конфігуратор має бути ФРЕЙМОМ з шарів (нижнє фото → стіна → кожен об'єкт окремо, об'єкти накладені на об'єкти), щоб заміна анімувалась пооб'єктно. Правило зафіксовано у пам'яті (layered-frame-configurators).
**Зроблено:** (1) gen-kitchen-layers.ts — 10 шарів straight-on елевації: room (порожня стіна+підлога) + uppers-modern/farm + lowers-modern/farm (БЕЗ стільниці) + counter-quartz/butcher (слаб+змішувач) + splash-classic/herring (текстури) + pendants. (2) game.ts: сцена = стек шарів (room→splash band→uppers→lowers→counter→pendants), generic setLayer з пер-шаровими анімаціями (style: slide-up/down, counter: drop+bounce, splash: left→right wipe), hook = style-toggle що демонструє механіку, reveal = full-bleed зум ×1.2 (нуль letterbox), endcard thumbnail = живий RenderTexture-знімок. (3) Адверсаріальний verify-workflow (3 лінзи × 10 скрінів) знайшов 25 дефектів; виправлено критичні: ROWS_X_SHIFT=115 (плита/духовка геть від центрального змішувача, apron-мийка за кадр — без подвійної мийки, витяжка над плитою), COUNTER_DY (butcher datum), SPLASH_TOP під шафи (без голої стіни), маска патернів свотчів (цегла текла за рамки), банер 0.86 (силует крана не плутається з текстом), thumb framing -43px (quartz не біле-на-білому), trust-рядки без дублювання копії.
**Перевірено:** build OK · qa-kitchen-flow обома шляхами (L: Modern/Quartz/Classic, R: Farm/Butcher/Herring) на ANGLE · особистий перегляд усіх 10 кадрів до і після фіксів.
**Деплой:** gh-pages portfolio-dream-kitchen-three-picks.html (82db8a8).

## Dream Kitchen — per-element choreography по ресерч-спеці (2026-06-11) 🆕
**Контекст:** оцінка юзера 3/10 по якості: «елементи великою групою, примітивні анімації»; наказ — ресерч (itch.io, makeover-ігри) перед роботою.
**Ресерч:** workflow 3 агенти × WebSearch (makeover-ігри Redecor/Playrix/Design Home, game-feel канон Juice-it-or-lose-it/Vlambeer, itch.io+GSAP stagger патерни) → 37 знахідок → синтез-спека: одиниця анімації = смуга/чанк; stagger 30-45ms; anticipation→out(back.in)→in(back.out 1.7-3.0 overshoot)→per-element FX; камера тільки на landing.
**Зроблено:** sliceRow (Texture frame sub-rects, 6 смуг на ряд, 5 на стільницю), cascadeSwap (анти-потік: верх L→R, низ R→L з 0.28s офсетом; пер-смугові sparkles, dust puff, camPunch 4px/1.006 на приземленні нижнього ряду), flipGridSwap для фартуха (6×3 чанків, діагональна flip-хвиля scaleX 0→1, shine після), counter як «заливка матеріалу» L→R з глінтом; reveal: roll-call по зонах (1→1.03→1 stagger 140ms + бурсти), пенданти ВМИКАЮТЬСЯ (ADD-glow з флікером), конфеті 26 квадів + punch. Після каскаду нарізка консолідується назад в один спрайт.
**Перевірено:** tsc OK · build OK · flow-QA L/R no errors · НОВИЙ qa-kitchen-anim.mjs (покадрові скріни кожні 150ms) — особисто переглянуто хвилю каскаду (450ms: ліві смуги farmhouse, праві ще modern), заливку стільниці, flip-хвилю плитки, glow пендантів на reveal.
**Деплой:** gh-pages cacb352.

## Dream Kitchen — reference-grade pass по ресерч-беклогу #2 (2026-06-11) 🆕
**Ресерч:** фоновий workflow (3 агенти × WebSearch: teardown'и makeover-playables, compositing-інтеграція 2D-колажів, interaction/conversion патерни) → 29 знахідок → пріоритезований беклог 8 пунктів.
**Зроблено (top-5 за impact/effort):** (1) AO/контактні тіні multiply-градієнтами: під верхніми шафами на фартух (0.28), контактна тінь нижніх на підлозі (0.38), тінь звису стільниці (0.30) — canvas-текстури, нуль фільтрів; (2) єдиний грейд (warm add 0.07) + radial-віньєтка multiply поверх усіх шарів = «одне фото»; (3) hook «Can you fix this kitchen?»: брудний before (multiply 0x96897a + 14 grime-плям) розчиняється у чисту сцену на 1.7s = перший before/after біт; (4) палець-підказка над лівою карткою після розчинення; (5) reveal-шторка: RenderTexture-снапшот before-стану стирається знизу вгору (0.85s) перед roll-call/конфеті — контраст показано двічі. Параллакс свідомо пропущено (тач, низький ROI).
**Перевірено:** tsc/build OK · flow-QA L/R no errors · anim-QA покадрово: hook-before з тінями ✓, шторка ✓, endcard зі снапшотом ✓.
**Деплой:** gh-pages (reference-grade pass).

## Fix-pass: 5 груп правок юзера (2026-06-12) 🆕
