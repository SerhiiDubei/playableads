...
## Епік: M — модульність (прибрати copy-paste і злиплення)

Зони/шаблони вже модульні (плагін-патерн). Лишилось прибрати дублювання й паралельні системи.
...
| M-03 | **(переосмислено)** `groups`/`flow`/`catalog` НЕ мертві — їх юзають games/recipes/catalog-doc. Питання не «видалити», а «уніфікувати 2 робочі системи (layered ↔ layouts/) чи лишити паралельними». Потребує рішення. | P2 | #modularity | Med | 3h | рішення | **blocked (decision)** |
| M-04 | Ігри (`build-game`/`build-connect`) стоять на `groups`/`flow`, не на контракті шаблонів. Доля залежить від M-03. | P2 | #modularity | Med | 2h | M-03 | **blocked (M-03)** |

**Контекст (виміряно):** `.title{}` копіюється в 7 файлах, reset — в 11, compact-card трюк — у 3; `knight` тягнеться в 10 шаблонах, але список ассетів зашитий у білдері.
**Порядок:** M-01 (найбільше копіпасту) → M-02 (ассети) → M-03 → M-04.
