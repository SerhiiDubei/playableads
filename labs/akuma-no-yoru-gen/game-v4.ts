...
async function main(){
  ...
  gsap.to(warrior, { rotation: 0.03, duration: 1.4, yoyo:true, repeat:-1, ease:"sine.inOut" });

  // blade trail graphics
  const trail = new Graphics(); fxLayer.addChild(trail);

  // ── UI ── (title = compact label top)
  const titleTxt = new Text({ text: cfg.copy.title, style:{ fontFamily:FONT, fontWeight:FW, fontSize:20, fill:num(C.text), stroke:{color:0x000000,width:3}, align:"center" }});
  ...
  const timeTxt = new Text({ text:"", style:{ fontFamily:FONT, fontWeight:FW, fontSize:24, fill:num(C.text), stroke:{color:0x000000,width:3} }});
  ...
  type Demon = { spr: Sprite; m: ReturnType<typeof makeApproach>; r: number; alive: boolean; kind: string; };
  type Special = { spr: Sprite; m: ReturnType<typeof makeArc>; r: number; alive: boolean; kind: string; };
  const demons: Demon[] = [];
  const specials: Special[] = [];
  const diff = new DifficultyController({ waitMin:0.5, waitMax:1.4, parallelMin:1, parallelMax:4 });
  ...
  function layout(){
    ...
    night.clear();
    night.rect(0,0,W,H).fill({color:0x000010, alpha:0.42});
    ...
    timeTxt.position.set(W/2, 44);
    titleTxt.position.set(W/2, 20);
    comboTxt.position.set(W/2, H*0.34);
  ...
  function spawnDemon(boss=false){
    ...
    const base = Math.min(W,H)*0.0014*(boss?1.9:1)*(0.85+Math.random()*0.35);
    spr.scale.set(base*0.6); gsap.to(spr.scale,{x:base,y:base,duration:0.4});
    spr.position.set(fx,fy);
    gsap.fromTo(spr,{alpha:0},{alpha:1,duration:0.4});
    gameLayer.addChild(spr);
    const m = makeApproach({ fromX:fx, fromY:fy, toX:tx, toY:ty, screenH:H, speedShPerSec: boss?0.10:(0.11+Math.random()*0.03), arriveRadius:30 });
    demons.push({ spr, m, r:Math.max(spr.width,spr.height)*0.5+50, alive:true, kind:boss?"boss":"demon" });
    sfx("spawn.wav");
  }

  function spawnSpecial(){
    const roll = Math.random();
    const kind = roll<0.4?"time rift": roll<0.75?"soul orb":"blood moon";
    ...
    const m = makeArc({ fromX:fx, fromY:H+60, screenH:H, timeAloftSec:2.8, apexFraction:0.6, angleDeg:randomLaunchAngle(Math.random()), spinRadPerSec:1.4 });
    specials.push({ spr, m, r:Math.max(spr.width,spr.height)*0.5+50, alive:true, kind });
  }

  // ── splat on background (accumulates) ──
  ...
  function killDemon(d:Demon){
    if(!d.alive) return;
    d.alive=false;
    addSplat(d.spr.x,d.spr.y,BLOOD);
    bloodBurst(d.spr.x,d.spr.y,BLOOD);
    // slash → split into halves (two skewed copies fly apart)
    for(const dir of [-1,1]){
      const half = new Sprite(d.spr.texture); half.anchor.set(0.5);
      half.position.set(d.spr.x,d.spr.y); half.scale.copyFrom(d.spr.scale);
      half.tint=0xbb6677; fxLayer.addChild(half);
      gsap.to(half,{ x:d.spr.x+dir*60, y:d.spr.y+40, rotation:dir*1.2, alpha:0, duration:0.45,
        onComplete:()=>{ if(half.parent) half.parent.removeChild(half); half.destroy(); }});
    }
  ...
  function loseLife(){
    ...
    // NOTE: lives==0 does NOT end the game; only the timer does.
  }

  // ── swipe handling (only stage listens; manual hit-test) ──
  app.stage.eventMode = "static";
  app.stage.hitArea = app.screen;
  let swiping=false; let px=0, py=0;
  const trailPts: {x:number,y:number}[] = [];

  function segDist(ax:number,ay:number,bx:number,by:number,qx:number,qy:number){
    const dx=bx-ax, dy=by-ay; const l2=dx*dx+dy*dy;
    if(l2===0) return Math.hypot(qx-ax,qy-ay);
  ...
  function hitTest(ax:number,ay:number,bx:number,by:number){
    for(const d of demons){ if(!d.alive) continue;
      if(segDist(ax,ay,bx,by,d.spr.x,d.spr.y) <= d.r){ killDemon(d); onSlay(); }
    }
  ...
  function onSlay(){
    ...
    lastKillT = now;
    const mult = chain>=5?4 : chain>=3?2 : 1;
  ...
  function startSwipe(e:any){ swiping=true; px=e.global.x; py=e.global.y; trailPts.length=0; trailPts.push({x:px,y:py});
    if(hintActive){ hintActive=false; hand.visible=false; gsap.killTweensOf(hand); gsap.to(hintTxt,{alpha:0,duration:0.4}); } }
  ...
  function endSwipe(){ swiping=false; trailPts.length=0; }
  ...
  // ── first action hint: hand sweeps from nearest demon through warrior ──
  function pointHintAtNearest(){
    ...
    if(best){
      hand.visible=true; gsap.killTweensOf(hand);
      hand.position.set(best.spr.x, best.spr.y);
      gsap.to(hand,{ x:warrior.x, y:warrior.y, duration:0.85, ease:"power1.inOut", yoyo:true, repeat:-1 });
    }
  }
  ...
  function endGame(){
    ...
    hand.visible=false; gsap.killTweensOf(hand);
    ...
    gsap.to(ctaBg.scale,{x:1.06,y:1.06,duration:0.6,yoyo:true,repeat:-1});
  }
  ...
  app.ticker.add(()=>{
    ...
    // spawning (density escalates, speed stays constant)
    spawnTimer -= dtReal;
    if(spawnTimer<=0 && elapsed<GAME_SEC){
      let count = diff.parallelCount();
      if(frenzyLeft>0) count += 2;
      for(let i=0;i<count;i++) spawnDemon(Math.random()<0.12);
      if(Math.random()<0.16) spawnSpecial();
      spawnTimer = (frenzyLeft>0?0.4:diff.spawnWaitSec());
    }

    // update demons (approach)
